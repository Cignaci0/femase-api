    ...
    try {
      ...
      if (idUsuario && !isNaN(idUsuario)) {
        ...
        if (autor) {
          ...
          // Función para extraer un valor comparable de campos primitivos, fechas u objetos (relaciones)
          ...
          for (const key of Object.keys(dtoTransformado)) {
            const valorAntiguo = parseValue(empleadoAntiguo[key as keyof typeof empleadoAntiguo]);
            const valorNuevo = parseValue(dtoTransformado[key]);
            
            if (dtoTransformado[key] !== undefined && valorAntiguo !== valorNuevo) {
              cambiosAuditoria.push(`${key} (de '${valorAntiguo}' a '${valorNuevo}')`);
            }
          }

          const textoCambios = cambiosAuditoria.length > 0
            ? `Cambios realizados: ${cambiosAuditoria.join(', ')}.`
            : `No se registraron cambios en los datos.`;

          const registroEvento = this.empleadoRepository.manager.create(RegistroEvento, {
            usuario: autor?.username,
            evento: `El usuario ${autor?.username} de la empresa ${autor?.empresa?.nombre_empresa || 'Sin Empresa'} ha actualizado los datos del empleado "${actualizada.nombres} ${actualizada.apellido_paterno} ${actualizada.apellido_materno || ''}" perteneciente a la empresa "${empGuardado?.empresa?.nombre_empresa || 'Desconocida'}". ${textoCambios}`,
